...
[ ] group houses, consider them as 1 (mouldable) puzzle piece        <- this is on a completely different level, ignore for now
...
LOGBOEK
    ...
    v 1.1:
        ...
        the family homes must move to make space for the large houses

        PERFORMANCE: around 50 ring increases before getting stuck
    ...
    v 1.2:
        ...
        NOTE: this also doesnt work, if two mansions clash they still jump around a lot for the exact same reason.
              we need something that generally solves an unsolved map
              - its A LOT of work to check all houses in real-time, so much work
                that im tempted to let the map just do another completely random construction

        PERFORMANCE: around 150 ring-increases before getting stuck
    ...
    OTHER THINGS DONE:
        - added an areConstraintsSatisfied method to map
    ...
    OTHER THINGS DONE:
        ...
        PERFORMANCE: 20 house case: 420 ring-increases, map value: 16.755.600
                     40 house case: 380 ring-increases, map value: 23.5 mil, water is no problem anymore
                     60 house case: 300 ring-increases, map value: 30.2 mil, water is not a problem anymore too,
                     giving a lot of space to mansions free's up enough space for the water squeeze in algorithm to work properly

        - note when a building is 'done'
"""

from Now.helpers2 import *

# choose 0, 1 or 2 to get 20, 40 or 60 houses
SELECTED_HOUSE_COUNT = 60 # HOUSE_COUNT[0] = 20
...
def main():
    ...
    for i in range(MACRO_LIMIT):
        ...
        # once every x ring-increases, print map
