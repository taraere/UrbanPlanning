...
class Rectangle(object):
    ...
    def __init__(self, OriginCoord, width, height):
        ...
        # get self' 4 boundary coordinates not used right now
...
##########################################################################
"""
r = Range(rangevalues)          # create range object
newRange = Range(rangevalue)    # create other range object
r.joinRange(newRange)           # combine ranges to r
"""
            """
            # randomize coord, but only use coord on the edge of a shape
            # hoe gaan we dit doen??
                # boundaries / rectangles moeten iets van een "get edges" functie hebben
                    # per rectangle: 4 "ranges"
                        EX:
                        Rectangle((0,0), 5, 7) =
                      (0,7) (5,7)
                        +-----+
                    ...
                    # these are correct, but how to we use em?
                    # can we join them together? not as easy as one might think
                    # we could also just store numberous coordinates... keep in mind half coordinates too, so LONG need to be used... thats a lot of storage
                        # ALSO, the ring size must be saved within a range object, when two houses clash, we must know who's ring's bigger...
                        # ALSO  how do we know in which direction the

                        AND SO:

                        rectangle.setEdgeRanges(aRingWidth):

                        (x, y, ringWidth, top/down/left/right)
                        ...
                        rectangle.getEdgeRanges():
                            return this stuff

                        # now when a house needs to be placed on one of these ranges, the house's got all info needed for correct placement

                    # join sets ranges together
                    # if ranges overlap, exclude the overlap from both ranges

                        - compare 2 ranges
                            - if 1 dimention is the same, and the others overlap in this fashion,
                              (0) - (7) and (3) - (10)
                              - swap them like this
                              (0) - (3) and (7) - (10)
                              - if range is (lower) == (upper)
                                    - delete range

                    # when i complete all of this, algorithm is "smart enough."
                # deze "random on edge" moet een array van die dingen krijgen

                # beslissingen:
                    # nieuw object: edge set??
                    # we could expand rectangle, but that class but remain as small as possible, its instanciated ALOT within algorithms
                                # QUESTION: in python, do methods of classes take up processing time when they are not used?

                    # hoe ga ik ranges uitdrukken / passen?
                    #
                    # overlap example:
